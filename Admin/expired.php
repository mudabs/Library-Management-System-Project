...
<div id="main">
  ...
  <div class = "container">
    ...
    if(isset($_SESSION['login_user']))
    {
      ...
      WHERE issue_book.approve ='Expired' OR issue_book.approve =''
      ...
      //Table Header
        if($row=mysqli_fetch_assoc($res)){ //Table header is displayed only when query is true
      ...
      {
        $c = $c+1; //counts the number of expired books
        ...
        <h2>Return Book</h2>
          <form action="" method="post">
              ...
              <input type="text" name="return" placeholder="New Return Date dd-mm-yyyy" required = "" class = "form-control"> <br>
              <button type="submit" name = "submit1" class = "btn btn-default">Return</button>
          </form>

        <!-- Query to renew a book-->

        <?php
          if(isset($_POST['submit1'])){

            //Updates the book status and return date
            $d = date("d-m-Y");
            mysqli_query($db,"UPDATE `issue_book`
            SET `approve`='Yes',`return`='$_POST[return]'
      ...
      //if count is 0 then the message is displayed
      if($c == 0){ 
          echo "<h2 style = 'text-align:center;'>There are no Expired Books</h2>";
      }
